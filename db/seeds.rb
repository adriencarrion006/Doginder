...
dogs1 = Dog.create!(user_id: user2.id, name: "lassie", breed:"border colier", centre_interet:"manger", fontawesome:"fas fa-utensils", description:"super gentille",location_cp:"06100")
...
dogs2 = Dog.create!(user_id: user3.id, name: "Beethoven", breed:"Saint-Bernard", centre_interet:"jouer avec le facteur", fontawesome:"fa-regular fa-mailbox", description:"il mange trop de croquettes",location_cp:"06400")
...
dogs3 = Dog.create!(user_id: user4.id, name: "Rantanplan", breed:"malinoi", centre_interet:"Courir en montagne", fontawesome:"fas fa-mountain", description:"trés fidéle",location_cp:"06100")
...
dogs4 = Dog.create!(user_id: user5.id, name: "Croc-Blanc", breed:"loup", centre_interet:"Faire la sieste", fontawesome:"fa-solid fa-face-sleeping", description:"il aime jouer dans la neige",location_cp:"06400")
...
dogs5 = Dog.create!(user_id: user6.id, name: "Bella-Chao", breed:"loupHusky siberien", centre_interet:"Creuser des trous", fontawesome:"fas fa-digging", description:"Elle porte bien son nom, Bella est très belle mais aussi très gentille et joueuse.",location_cp:"06400")
...
dogs6 = Dog.create!(user_id: user7.id, name: "Caramel", breed:"Bouledogue Américain", centre_interet:"Chasser les chats", fontawesome:"fas fa-cat", description:"Caramel est un grand garçon, tout en force !
...
dogs7 = Dog.create!(user_id: user8.id, name: "Cricket", breed:"Croisé / Autre", centre_interet:"Ronger mon nonos", fontawesome:"fas fa-bone", description:"Cricket est un chien qui peut se montrer un peu timide au départ et assez câlin une fois qu'il connait.
...
dogs8 = Dog.create!(user_id: user9.id, name: "Ewelina", breed:"Berger", centre_interet:"Jouer à la baballe", fontawesome:"fas fa-tennis-ball", description:"Ewelina est une chienne sensible",location_cp:"06400")
...
dogs10 = Dog.create!(user_id: user10.id, name: "Hijo", breed:"croisé ", centre_interet:"Agility", fontawesome:"fas fa-running", description:"Hijo a énormément de mal à faire confiance aux humains. Craintif, méfiant, il aura besoin de temps et d'une approche très douce pour se révéler sous son vrai jour.
...
dogs11 = Dog.create!(user_id: user11.id, name: "Jany", breed:"American staffordshire", centre_interet:"Suivre une piste", fontawesome:"fa-regular fa-road", description:"Jany est un jeune chienne, dynamique et affectueuse.
...
dogs13 = Dog.create!(user_id: user13.id, name: "Magnum", breed:"Drahthaar", centre_interet:"Mordre le facteur", fontawesome:"fa-regular fa-mailbox", description:"Magnum s'entend avec ses congénères mais il a ses têtes. Il ne s'entend pas avec les chats.
...
dogs14 = Dog.create!(user_id: user14.id, name: "Nicky", breed:"Croisé / Dogue argentin", centre_interet:"Jouer au freesbee", fontawesome:"fa-solid fa-flying-disc", description:"Nicky est un chien affectueux, qui aime son confort et qui a toujours envie d'apprendre et de faire plaisir.
...
dogs15 = Dog.create!(user_id: user15.id, name: "Odda", breed:"Braque", centre_interet:"chasser les oiseaux", fontawesome:"fas fa-dove", description:"Odda est un chien très dynamique à qui il faudra apprendre à se canaliser en méthode positive. Il aura besoin d'être énormément dépensé, autant physiquement qu'intellectuellement, au risque de rencontrer des problèmes de comportement.",location_cp:"06400")
...
dogs16 = Dog.create!(user_id: user16.id, name: "Romeo", breed:"Croisé / Fox terrier poil dur", centre_interet:"nager en riviere", fontawesome:"fas fa-swimmer", description:"Roméo est un gentil petit loulou très joueur et affectueux !",location_cp:"06400")
...
dogs17 = Dog.create!(user_id: user17.id, name: "Titounet", breed:"Berger", centre_interet:"courir en montagne", fontawesome:"fas fa-mountain", description:"Titounet est un chien qui a encore beaucoup à apprendre. Très dynamique et joueur.",location_cp:"06400")
...
dogs18 = Dog.create!(user_id: user18.id, name: "Bayron", breed:"croisé", centre_interet:"se rouler dans l'herbe fraiche", fontawesome:"fas fa-flower-tulip", description:"Bayron n'est pas un papy tranquille, il déborde encore d'énergie & devra pouvoir bénéficier d'une bonne dépense quotidienne, car monsieur à encore énormément d'énergie à revendre ",location_cp:"06300")
